...
export async function create(req: Request, res: Response) {
	try {
		...
		const {
			name, fileLink, instructorId, courseId, categoryId, degreeId,
		} = req.body as ReceivedExam;
		...
		if (err instanceof QueryFailedError) {
			...
			if (code === '23502') return res.status(404).send(`${message} => ${detail}`);
		}
...
export async function getWithInstructorIdByCategory(req:Request, res:Response) {
	try {
		...
		const instructorId = parseInt(paramInstructorId.toString(), 10);
...
export async function getWithCourseIdByCategory(req:Request, res:Response) {
	try {
		...
		if (joiError) throw joiError;
		const courseId = parseInt(paramCourseId.toString(), 10);
